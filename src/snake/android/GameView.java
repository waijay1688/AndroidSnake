...
public class GameView extends SurfaceView implements Callback{
	...
	public GameView(Context context)
	{
		...
		step_count=Snake_Data.STEP;
		if(Snake_Data.BCLASSIC||Snake_Data.BTIME)
		snake=new Snake_Classic();
		else
			...
			if(Snake_Data.BTIME)
			{
				...
				{	
					try
					{
						...
						{							
							c.drawBitmap(background,null,r, null);							
							if(step_count-(int)time_use<=0)
								run=false;
							
							drawNum((step_count-(int)time_use),26*18,0,c,time);
							temp_score=snake.getScore();
							drawNum(temp_score,5*18,0,c,score);
							...
							time_use+=Snake_Data.TEMP_FRENCUNCY/1000.0;						
						}						
					}
					catch (Exception e) 
					{
